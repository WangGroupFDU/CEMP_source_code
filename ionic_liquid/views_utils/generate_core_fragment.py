...
def create_excel_from_lists(
    cation_core_list, 
    cation_backbone_list, 
    anion_core_list, 
    anion_backbone_list, 
    output_dir="."
):
...
def collect_unique_cores_and_backbones(
    df_cation_core_backbone: pd.DataFrame,        
    df_anion_core_backbone: pd.DataFrame          
) -> Tuple[List[str], List[str], List[str], List[str]]:
    required = {"Core_SMILES", "Backbone_SMILES"} 

    
    for name, df in [("df_cation_core_backbone", df_cation_core_backbone), ("df_anion_core_backbone", df_anion_core_backbone)]:
        missing = required - set(df.columns)      
        if missing:                               
            raise ValueError(f"{name} 缺少必要列：{missing}")

    
    def _split_smiles_series(col: pd.Series) -> List[str]:
        out: List[str] = []                       
        for val in col:                           
            if pd.isna(val):                      
                continue
    ...
    cation_cores = _split_smiles_series(df_cation_core_backbone["Core_SMILES"])         
    cation_backbones = _split_smiles_series(df_cation_core_backbone["Backbone_SMILES"]) 

    
    anion_cores = _split_smiles_series(df_anion_core_backbone["Core_SMILES"])           
    anion_backbones = _split_smiles_series(df_anion_core_backbone["Backbone_SMILES"])   
    ...
    return cation_cores, cation_backbones, anion_cores, anion_backbones  

def out(name, output_dir) -> str:
    return str(Path(output_dir) / name)  

def preprocess_core_fragment(df_cation_core_backbone, df_anion_core_backbone, output_dir="."):
    cation_cores, cation_backbones, anion_cores, anion_backbones = collect_unique_cores_and_backbones(df_cation_core_backbone, df_anion_core_backbone)
    ...
    create_excel_from_lists(cation_cores, cation_backbones, anion_cores, anion_backbones, output_dir=output_dir)

    filter_molecule(out('Cation_core.xlsx',output_dir))         
    filter_molecule(out('Cation_backbone.xlsx',output_dir))     
    filter_molecule(out('Anion_core.xlsx',output_dir))          
    filter_molecule(out('Anion_backbone.xlsx',output_dir))      

    filter_virtual_site_number(out('Cation_core.xlsx',output_dir))
    filter_virtual_site_number(out('Cation_backbone.xlsx',output_dir))
    filter_virtual_site_number(out('Anion_core.xlsx',output_dir))
    filter_virtual_site_number(out('Anion_backbone.xlsx',output_dir))

    Cation_core_data_count      = count_excel_rows(out('Cation_core.xlsx',output_dir))        
    Cation_backbone_data_count  = count_excel_rows(out('Cation_backbone.xlsx',output_dir))
    Anion_core_data_count       = count_excel_rows(out('Anion_core.xlsx',output_dir))
    Anion_backbone_data_count   = count_excel_rows(out('Anion_backbone.xlsx',output_dir))

    print(
        f"Processing completed. Generated files:\n"
        f"- '{out(output_dir,'Cation_core.xlsx')}' ({Cation_core_data_count} entries)\n"
        f"- '{out(output_dir,'Cation_backbone.xlsx')}' ({Cation_backbone_data_count} entries)\n"
        f"- '{out(output_dir,'Anion_core.xlsx')}' ({Anion_core_data_count} entries)\n"
        f"- '{out(output_dir,'Anion_backbone.xlsx')}' ({Anion_backbone_data_count} entries)"
    )
