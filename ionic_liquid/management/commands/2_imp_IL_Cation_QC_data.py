...
from ionic_liquid.models import Cation_QC_data
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        Cation_QC_data.objects.all().delete()

        
        df = pd.read_excel('ionic_liquid/management/commands/HTQC_cation.xlsx')

        
        for _, row in df.iterrows():
            entry=Cation_QC_data(
                Name=row['Name'],
                SMILES=row['SMILES'],
                Cation_type=row['SMILES_type'],
