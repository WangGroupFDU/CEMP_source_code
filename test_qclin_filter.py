...
def main():
    """
    功能目的：手动比较电池 MongoDB Qclin/Qdlin 查询逻辑。
    输入参数：无，默认连接本机 MongoDB battery_db.battery_cycles。
    返回值：无，向终端打印样本、统计和性能对比。
    关键流程：先检查单条样本和 Qclin 覆盖率，再比较逐电池查询与聚合查询。
    边界情况：公开版不默认提供 MongoDB，本脚本只能手动运行，不能在测试导入时连接数据库。
    """
    ...
    for bat_key in bat_keys_full:
        ...
        cycles = list(
            cycles_col.find(query, {"Qdlin": 1, "cycle_num": 1, "_id": 0}).sort("cycle_num")
        )
        if cycles:
            cycle_data = []
            for c in cycles:
